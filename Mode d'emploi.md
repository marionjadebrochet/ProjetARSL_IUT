...
Sur la première page du site on peut trouver deux informations principales : une description du **SIAO** et des informations sur les **accueils de jour** des différentes organismes.
...
Il existe un compte Administrateur qui permet de gérer toute la base de données du site internet, et lui seul à l'accès à la création et suppression de nouveaux utilisateurs et organismes. Son pseudo est **admin** eet son mot de passe **projetarsl**. il faudra vous connecter avec lui pour pouvoir créer les premiers **administrateurs d'organisme**. Eux-même pourront ensuite créer les compte utilisateurs de leur organisme par la suite. Les détails sont écrits dans la partie **membres** un peu plus bas.
...
Les **comptes** déjà existant sont admin, ARSL avec le pseudo **arsls** avec le mot de passe **projetarsl** et ETAT avec le pseudo **etatvisiteur** et le mot de passe **etat**.
...
L'administrateur peut décider d'ajouter ou de supprimer des organismes. En raison de quelques problèmes techniques, il n'est aps encore possible d'ajouter un logo à une association.
...
Vous devrez ensuite vous rendre sur un site qui sera affiché sur la page AJOUTER. Après avoir été redirigé, vous devrez simplement rentrer l'adresse du lieu et copié/collé les coordonnées latitude et longitude dans les champs correspondants.

SUPPRIMER

Vous pouvez supprimer un lieu si vous avez fait une erreur ou si le lieu ne convenait pas. ATTENTION ! La suppression d'un lieu supprimera aussi les données (du lieu) associées au accueils de jour et maraude qui lui étaient associés. 
...
Vous pouvez sur cette page voir la liste des services des accueils de jour de votre organisme et donc vous pouvez les supprimer. Les supprimer n'aura aucun impact que sur le service supprimé.
