      ...
      <fieldset>
        ...
        <div class="row">
          ...
          <select required v-model="role">
            <option v-for="role in roles" :key="role.id" :value="role.id" v-if="role.name != 'Admin' && role.name != 'adminDev' && role.name != 'Authenticated' && role.name != 'Public'">{{role.name}}</option>
          </select>
        </div>
